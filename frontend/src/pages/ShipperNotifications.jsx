...
export default function ShipperNotifications() {
  ...
  const [dismissedIds, setDismissedIds] = useState(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem('shipper_dismissed_notifications') || '[]'));
    } catch {
      return new Set();
    }
  });
  ...
  const handleMarkAsRead = (id) => {
    ...
    localStorage.setItem('shipper_dismissed_notifications', JSON.stringify([...next]));
  };
        ...
        {!notificationsEnabled ? (
          ...
          <div className="space-y-4">
            {visibleAlerts.map((notif) => (
              <div key={notif._id} className="p-4 rounded-xl border bg-white shadow-sm flex items-start gap-4 border-l-4 border-l-purple-600 transition-all duration-300 hover:shadow-md">
