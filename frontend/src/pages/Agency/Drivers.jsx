...
function DriverCard({ driver, busy, onRemove }) {
  return (
    <div className="relative rounded-xl border border-gray-100 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[11px] text-gray-400 font-medium">{driver.phone}</span>
          </div>
        </div>
        <span
...
function DriverAvatar({ photo, name, size = 'w-10 h-10' }) {
  if (photo) {
    return <img src={photo} alt={name} className={`${size} rounded-full object-cover border border-gray-100`} />;
  }
  return (
    <div className={`${size} bg-gray-50 border border-gray-100 rounded-full flex items-center justify-center text-gray-500 font-bold`}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/45 backdrop-blur-sm transition-all">
      <div className="bg-white rounded-2xl max-w-md w-full shadow-xl border border-gray-100 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <h2 className="text-base font-bold text-gray-900">Onboard Driver</h2>
          ...
          <form onSubmit={handleSearch} className="space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 text-sm rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#249B74]/20 focus:border-[#249B74] transition disabled:bg-gray-50 disabled:text-gray-400"
              />
              {preview ? (
                <button
                  type="button"
                  onClick={resetSearch}
                  className="px-4 py-2 text-xs font-bold border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition"
                >
                  Change
                </button>
              ) : (
                <button
                  type="submit"
                  disabled={searching}
                  className="px-4 py-2 text-xs font-bold bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition disabled:opacity-60"
                >
          ...
          {preview && (
            <div className="rounded-xl border border-gray-100 bg-gray-50/60 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">{preview.driver.name}</h3>
                  <span className="text-[11px] text-gray-400 font-medium">{preview.driver.phone}</span>
                </div>
              </div>
              ...
              {preview.alreadyLinkedElsewhere ? (
                ...
                <button
                  ...
                  disabled={linking}
                  className="mt-4 w-full py-2.5 text-xs font-bold bg-[#249B74] hover:bg-[#1C4E3A] text-white rounded-lg transition shadow-sm disabled:opacity-60"
                >
